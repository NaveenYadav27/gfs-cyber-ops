...
import {
  Server, Shield, Cloud, Database, Globe, Wifi, Monitor, Lock, X, ArrowRight,
  AlertTriangle, Clock, Cpu, HardDrive, Network, Boxes,
} from 'lucide-react';
    ...
    >
      <div className="p-6 space-y-5">
        ...
        <div className="p-3 rounded-lg bg-[var(--color-gfs-surface)] border border-[var(--color-gfs-border-light)]">
          <div className="flex items-center justify-between">
            <span className="text-xs text-[var(--color-gfs-text-muted)]">System Status</span>
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            { label: 'IP / Endpoint', value: system.ip },
            ...
            { label: 'Last Patch', value: system.lastPatch },
          ].map((f) => (
            <div key={f.label} className="p-2.5 rounded-lg bg-[var(--color-gfs-surface)]">
              <div className="text-[9px] text-[var(--color-gfs-text-muted)] uppercase tracking-wider">{f.label}</div>
              <div className="text-[11px] text-[var(--color-gfs-text)] font-mono mt-0.5 capitalize">{f.value}</div>
            </div>
          ))}
        </div>
        ...
        {system.osVersion && (
          <div>
            <h4 className="text-[10px] text-[var(--color-gfs-text-muted)] uppercase tracking-wider mb-2">Operating System / Version</h4>
            <p className="text-xs text-[var(--color-gfs-text)] font-mono">{system.osVersion}</p>
          </div>
        )}

        <div>
          <h4 className="text-[10px] text-[var(--color-gfs-text-muted)] uppercase tracking-wider mb-2">Ports & Services</h4>
          <div className="flex flex-wrap gap-1.5 mb-2">
            {system.ports.map((port) => (
              <Badge key={port} variant="default">:{port}</Badge>
            ))}
          </div>
          <div className="flex flex-wrap gap-1.5">
            {system.services.map((svc) => (
              <Badge key={svc} variant="accent">{svc}</Badge>
            ))}
          </div>
        </div>
        ...
        <div>
          <h4 className="text-[10px] text-[var(--color-gfs-text-muted)] uppercase tracking-wider mb-2">Last Scan</h4>
          <p className="text-xs text-[var(--color-gfs-text-secondary)] flex items-center gap-1">
            <Clock className="w-3 h-3" /> {system.lastScan}
          </p>
        </div>
      </div>
    </motion.div>
...
export function InfrastructureOverview() {
  ...
  const filtered = filterCategory === 'all' ? ENTERPRISE_SYSTEMS : ENTERPRISE_SYSTEMS.filter((s) => s.category === filterCategory);

  return (
    <div className="space-y-4">
      <PageHeader
        icon={<Boxes className="w-5 h-5 text-[var(--color-gfs-accent)]" />}
        title="Enterprise Infrastructure"
        subtitle="Interactive map of all GFS systems, services, and infrastructure components"
        badge={<div className="flex items-center gap-3 text-[10px] text-[var(--color-gfs-text-muted)]">
          <span className="flex items-center gap-1"><StatusIndicator status="online" size="sm" /> Online: {ENTERPRISE_SYSTEMS.filter(s => s.status === 'online').length}</span>
          <span className="flex items-center gap-1"><StatusIndicator status="degraded" size="sm" /> Degraded: {ENTERPRISE_SYSTEMS.filter(s => s.status === 'degraded').length}</span>
        </div>}
      />
        ...
        {categories.map((cat) => (
          <button key={cat} onClick={() => setFilterCategory(cat)}
            className={`px-3 py-1.5 rounded-lg text-[11px] capitalize transition-colors ${
              filterCategory === cat ? 'bg-[var(--color-gfs-accent-dim)] text-[var(--color-gfs-accent)] border border-[var(--color-gfs-accent)]/30' : 'bg-[var(--color-gfs-surface)] text-[var(--color-gfs-text-muted)] hover:text-[var(--color-gfs-text)] border border-transparent'
            }`}>
            {cat}
          </button>
        ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
        {filtered.map((system, i) => {
          const Icon = categoryIcons[system.category] || Server;
          return (
            <Card key={system.id} delay={i * 0.03} hover onClick={() => setSelectedSystem(system)} className="!p-4 group">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-[var(--color-gfs-text-muted)] mt-0.5">{system.type}</p>
              ...
              <div className="flex items-center justify-between mt-3 pt-3 border-t border-[var(--color-gfs-border-light)]">
                <div className="flex items-center gap-2">
                  <Badge variant={system.criticality === 'critical' ? 'critical' : system.criticality === 'high' ? 'high' : 'medium'}>{system.criticality}</Badge>
        ...
        {selectedSystem && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="fixed inset-0 bg-black/50 z-40" onClick={() => setSelectedSystem(null)} />
